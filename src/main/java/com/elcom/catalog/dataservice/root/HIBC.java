...
@Entity
@Table(name="HIBC")
public class HIBC implements Serializable {
	private static final long serialVersionUID = 1L;

	@Id
	@SequenceGenerator(name="HIBC_ID_GENERATOR", sequenceName="HIBC_ID_SEQ", allocationSize = 1)
	@GeneratedValue(strategy=GenerationType.SEQUENCE, generator="HIBC_ID_GENERATOR")
	@Column(name="ID", unique=true, nullable=false, length=10)
	private Long id;

	@Column(name="HIBC", length=50, nullable=false)
	private String hibc;

	@ManyToOne
	@JoinColumn(foreignKey = @ForeignKey(name = "FK_HIBC_IMPORT_TRANSACTION_ID"), name="IMPORT_TRANSACTION_ID")
